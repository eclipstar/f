...
const SignupSchema = Yup.object().shape({
  ...
  year: Yup.number().min(1950, 'Anio no puede ser menor 1950').max(new Date().getFullYear(), `El anio no debe ser mayor a ${new Date().getFullYear()}`).required('El anio es requerido'),
          ...
          >
            {({ handleChange, handleBlur, handleSubmit, values, setFieldValue,errors, touched }: any) => (
              <View style={styles.formContainer}>
                  ...
                  <Text style={styles.labelDecription}>Queremos conocerte y saber más de tí para brindarte una atención más personalizada.</Text>
                <View style={styles.controlsContainer}>
                    ...
                    <View style={{...styles.pickerWapper2}}>
                      ...
                      {errors.location && touched.location && <Text style={{...styles.errorText,...styles.spacing}}>{errors.location}</Text>}
                    <View style={styles.buttonBox}>
                      <Button appearance='filled' color={colors.primary} handleClick={handleSubmit} rounded >SIGUIENTE</Button>
                    </View>
                </View>
              </View>
            )}
...
const styles = StyleSheet.create({
  ...
  pickerWapper: {
    borderRadius: 10,
    overflow: 'hidden', // Ensure the borderRadius applies to the contents
